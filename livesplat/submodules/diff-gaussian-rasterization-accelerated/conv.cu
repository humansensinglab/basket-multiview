...
template <int C>
__device__ float get_pix_value(const float *img, const int c, const int y,
                               const int x, const int H, const int W) {
  if (x >= W || y >= H || x < 0 || y < 0) {
    return 0.0f;
  } else {
 ...
 */
template <int C>
__global__ void transposeCUDA(float *odata, float *idata, int width,
                              int height) {
...
__global__ void separableConvCUDA(const float *__restrict__ input,
                                  float *__restrict__ output, const int H,
                                  const int W) {
...
__global__ void convCUDA(const float *__restrict__ input,
                         float *__restrict__ output, const int H, const int W) {
  auto block = cg::this_thread_block();
  const int pix_y =
      block.group_index().y * block.dim_threads().y + block.thread_index().y;
  const int pix_x =
...
__global__ void ssimrestCUDA(int N, float C1, float C2, float *mu1, float *mu2,
                             float *mim, float *mom, float *mu2_sq,
                             float *sigma2_sq, float *ssim_map) {
...
__global__ void ssimrest_backCUDA(int N, float C1, float C2, float *mu1_,
                                  float *mu2_, float *mim_, float *mom_,
                                  float *mu2_sq_, float *sigma2_sq_, float *dL,
                                  float *dL_dmu1, float *dL_dmim,
                                  float *dL_dmom) {
  int idx = threadIdx.x + blockDim.x * blockIdx.x;
  if (idx >= N)
    return;
...
torch::Tensor ssimrest(float C1, float C2, torch::Tensor &mu1,
                       torch::Tensor &mu2, torch::Tensor &mim,
                       torch::Tensor &mom, torch::Tensor &mu2_sq,
...
ssimrest_back(float C1, float C2, torch::Tensor &mu1, torch::Tensor &mu2,
              torch::Tensor &mim, torch::Tensor &mom, torch::Tensor &mu2_sq,
              torch::Tensor &sigma2_sq, torch::Tensor &dL) {
  int N = mu1.size(0) * mu1.size(1) * mu1.size(2);
...
template <int C>
__device__ void load_into_shared(float pixels[BY + 10][BX + 10], float *input1,
                                 float *input2, int H, int W, int i,
                                 int subtract = 0) {
  auto block = cg::this_thread_block();
...
__device__ float do_conv(float pixels[BY + 10][BX + 10], int H, int W,
                         bool sq = false) {
  auto block = cg::this_thread_block();
  int local_y = block.thread_index().y + 5;
  int local_x = block.thread_index().x + 5;
...
template <int CH>
__global__ void fusedssimCUDA(int H, int W, float C1, float C2, float *img1,
                              float *img2, float *ssim_map) {
  auto block = cg::this_thread_block();
...
template <int CH>
__global__ void fusedssim_backwardCUDA(int H, int W, float C1, float C2,
                                       float *img1, float *img2, float *dL_dmap,
                                       float *dL_dimg1) {
  auto block = cg::this_thread_block();
  const int pix_y =
