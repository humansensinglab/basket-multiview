...
from utils.dynamic_utils import (create_output, get_timestamps,
                                 ...
                                 indices_from_file, SkeletonGlobal,
                                 read_skin_file, transform_gaussians_by_skeleton)
...
def training_frame(
    ...
    binding_skeleton, 
    animated_skeleton,
    gs_skn_weights : torch.Tensor,
    gs_skn_weights_inds : torch.Tensor,
    ):
    ...
    if model_params.noisy_skeleton:
        num_skeleton_iters = 60
        gaussian_model._xyz.requires_grad_(False)
        gaussian_model._features_dc.requires_grad_(False)
        gaussian_model._features_rest.requires_grad_(False)
        gaussian_model._opacity.requires_grad_(False)
        gaussian_model._scaling.requires_grad_(False)
        gaussian_model._rotation.requires_grad_(False)
    else:
        num_skeleton_iters = 1
        transform_gaussians_by_skeleton(gaussian_model, binding_skeleton, animated_skeleton,
                                    gs_skn_weights, gs_skn_weights_inds, chains)
    
    
    checkpoint_path = os.path.join(path_to_output_folder, "checkpoints")
    gaussian_model.save_ply(os.path.join(checkpoint_path, f"{frame_idx:04d}_before_optimization.ply"))
    ...
    while iteration <= num_iters:
        ...
        if iteration < num_skeleton_iters:
            transform_gaussians_by_skeleton(gaussian_model, binding_skeleton, animated_skeleton,
                                    gs_skn_weights, gs_skn_weights_inds, chains)
        elif iteration == num_skeleton_iters:
            gaussian_model.make_parameters_leaf_tensors()
        ...
        if torch.isnan(loss_dict.get('total_loss', torch.tensor(0.0))) or torch.isinf(loss_dict.get('total_loss', torch.tensor(0.0))):
            print(f"[Warning] NaN/Inf detected at iteration {iteration}! Skipping step.")
            loss_object.optimizer.zero_grad()
            iteration += 1
            continue

        if visibility_filter.sum() > 0.1 * gaussian_model._xyz.shape[0]: 
            # g = start_range('backward')
        ...
        with torch.no_grad():
            ...
            if iteration < num_iters:
                if iteration < num_skeleton_iters:
                    animated_skeleton.optimize_step()
    ...
    animated_skeleton.save_skl_file(os.path.join(checkpoint_path, f"{frame_idx:04d}.skl"))
...
def training(
    ...
    if model_params.dataset_type == 'basket_mv':
        ply_path = Path(model_params.first_frame_ply).resolve()
        dataset_root = ply_path.parents[5]
        assoc_path = ply_path.parents[2] / "assoc.txt"
        binding_skl_path = dataset_root / "skls" / "0000.skl"
        skin_weight_path = dataset_root / "skinning_weights.skin"
        dynamic_skl_paths = Path(model_params.source_path) / "skeleton" / model_params.object_key
        skel_scale = 1 / 100.0
    else:
        raise NotImplementedError(f"Dataset type {model_params.dataset_type} not implemented yet.")
    
    gs2vert = indices_from_file(assoc_path).cuda()
    # Initialize Gaussianss
    ...
    post_proc = post_proc_factory(
        type=post_proc_params['type'], 
        conf=post_proc_params['conf'])
    motion_info = None

    # Load all the datasets all at once
    ...
    conf['erosion_kernel_size'] = 2
    conf['object_key'] = model_params.object_key if model_params.object_key else None

    # Load binding skeleton, skinning weight and gaussian-vertex association
    binding_skeleton = SkeletonGlobal(binding_skl_path, device='cuda', requires_grad=False, skel_scale=skel_scale)
    vert_skn_weights, vert_skn_weights_inds = read_skin_file(skn_path=skin_weight_path,
                                                             skl_path=binding_skl_path)
    ...
    for t_idx, timestamp in paired_timestamps:
        ...
        animated_skeleton = SkeletonGlobal(dynamic_skl_paths / f'{t_idx:04d}.skl', device='cuda', requires_grad=model_params.noisy_skeleton)
        
        motion_info = post_proc.process(
            last_gauss_model=gaussian_model, 
            motion_info=motion_info,
            frame_idx=t_idx)
        ...
        training_frame(
            ...
            frame_idx=t_idx,
            num_total_frames=len(paired_timestamps),
            test_iters=test_iters,
            binding_skeleton=binding_skeleton,
            animated_skeleton=animated_skeleton,
            gs_skn_weights=gs_skn_weights,
            gs_skn_weights_inds=gs_skn_weights_inds,
        )
...
if __name__ == "__main__":
    ...
    post_proc_params = get_post_proc_config(type='dynamic')
