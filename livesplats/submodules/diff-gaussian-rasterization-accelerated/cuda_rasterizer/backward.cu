...
__global__ void preprocessCUDA(
    int P, int D, int M, const float3 *means, const int *radii, const float *dc,
    ...
    glm::vec3 *dL_dscale, glm::vec4 *dL_drot, bool precomp) {
  auto idx = cg::this_grid().thread_rank();
  if (idx >= P || !(radii[idx] > 0))
    return;
  ...
  if (!precomp) {
    ...
    if (shs)
      ...
      computeColorFromSH(idx, D, M, (glm::vec3 *)means, *campos, dc, shs,
                         clamped, (glm::vec3 *)dL_dcolor,
                         (glm::vec3 *)dL_dmeans, (glm::vec3 *)dL_ddc,
                         (glm::vec3 *)dL_dsh, false);
...
__global__ void PerGaussianRenderCUDA(
    ...
    int W, int H, int B, const uint32_t *__restrict__ per_tile_bucket_offset,
...
template <uint32_t C>
__global__ void PerGaussianRenderPreCUDA(
    const uint2 *__restrict__ ranges, const uint32_t *__restrict__ point_list,
    int W, int H, int B, const uint32_t *__restrict__ per_tile_bucket_offset,
    const uint32_t *__restrict__ bucket_to_tile,
    const float *__restrict__ sampled_T, const float *__restrict__ sampled_ar,
  ...
  for (int i = 0; i < BLOCK_SIZE + 31; ++i) {
    ...
    if (valid_splat && valid_pixel && 0 <= idx && idx < BLOCK_SIZE) {
      ...
      const float dchannel_dcolor = alpha * T;
      for (int ch = 0; ch < C; ++ch)
        Register_dL_dcolors[ch] += dchannel_dcolor * dL_dpixel[ch];
...
template <uint32_t C>
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderLoadBalancingCUDA(
    const uint2 *__restrict__ ranges, const uint32_t *__restrict__ point_list,
    ...
    const float *__restrict__ dL_dpixels, float3 *__restrict__ dL_dmean2D,
    float4 *__restrict__ dL_dconic2D, float *__restrict__ dL_dopacity,
    float *__restrict__ dL_dcolors, const uint2 *__restrict__ tiles_ID,
    const int num_tiles, float *__restrict__ pixel_tr, const bool lb_last_set,
    const short *__restrict__ lb_last_num) {

  const int Tile_partition = 64;
  ...
  int global_thread = idx * BLOCK_SIZE + block.thread_rank();
  float epsilon = 1e-4f;

  uint32_t tile_id = tiles_ID[idx].x;
  uint32_t order_in_tile = tiles_ID[idx].y;

  uint2 range = ranges[idx];
  uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
  uint2 tile_2d = {tile_id % horizontal_blocks, tile_id / horizontal_blocks};
  uint2 pix_min = {tile_2d.x * BLOCK_X, tile_2d.y * BLOCK_Y};
  ...
  const float T_final =
      inside ? pixel_tr[(idx)*BLOCK_SIZE + block.thread_rank()] : 0.0f;
  float T = T_final;

  // if (lb_last_set) {
  //   if ((order_in_tile == 0 && fabs(T) < epsilon) ||
  //       (order_in_tile != 0 && fabs(T) < epsilon &&
  //        fabs(pixel_tr[(idx - 1) * BLOCK_SIZE + block.thread_rank()]) >
  //            epsilon)) {
  //     toDo = lb_last_num[pix_id];
  //     T = 0.0001;
  //     range.y = range.x + toDo;
  //   }
  // }
  ...
  if (block.thread_rank() < toDo) {
    const int coll_id = point_list[range.y - block.thread_rank() - 1];
...
void BACKWARD::preprocess(
    ...
    const float *opacities, const glm::vec3 *scales, const glm::vec4 *rotations,
    ...
    const float3 *dL_dmean2D, const float *dL_dconic, float *dL_dopacity,
    glm::vec3 *dL_dmean3D, float *dL_dcolor, float *dL_dcov3D, float *dL_ddc,
    float *dL_dsh, glm::vec3 *dL_dscale, glm::vec4 *dL_drot, bool precomp,
    cudaStream_t stream) {
  ...
  if (!precomp) {
    ...
    computeCov2DCUDA<<<(P + 255) / 256, 256, 0, stream>>>(
        ...
        viewmatrix, opacities, dL_dconic, dL_dopacity, (float3 *)dL_dmean3D,
        dL_dcov3D);
  }
  ...
  preprocessCUDA<NUM_CHAFFELS><<<(P + 255) / 256, 256, 0, stream>>>(
      ...
      dL_ddc, dL_dsh, dL_dscale, dL_drot, precomp);
}

void BACKWARD::render(const dim3 grid, dim3 block, const uint2 *ranges,
                      ...
                      const float *sampled_ar, const float *bg_color,
  ...
  PerGaussianRenderCUDA<NUM_CHAFFELS>
      <<<((B * 32) + THREADS - 1) / THREADS, THREADS, 0, stream>>>(
...
void BACKWARD::renderPre(const dim3 grid, dim3 block, const uint2 *ranges,
                         const uint32_t *point_list, int W, int H, int R, int B,
                         const uint32_t *per_bucket_tile_offset,
                         const uint32_t *bucket_to_tile, const float *sampled_T,
                         const float *sampled_ar, const float *bg_color,
                         ...
                         float3 *dL_dmean2D, float4 *dL_dconic2D,
                         float *dL_dopacity, float *dL_dcolors,
                         const uint2 *tiles_ID, const int num_tiles,
                         float *pixel_tr, cudaStream_t stream,
                         const bool use_load_balancing, const bool lb_last_set,
                         const short *lb_last_num) {

  if (use_load_balancing) {

    int dimx = ceil(sqrt(num_tiles));
    dim3 tile_grid = {dimx,
                      (uint32_t)ceil(static_cast<float>(num_tiles) / dimx), 1};

    renderLoadBalancingCUDA<NUM_CHAFFELS><<<tile_grid, block, 0, stream>>>(
        ranges, point_list, W, H, bg_color, means2D, conic_opacity, colors,
        nullptr, nullptr, dL_dpixels, dL_dmean2D, dL_dconic2D, dL_dopacity,
        dL_dcolors, tiles_ID, num_tiles, pixel_tr, lb_last_set, lb_last_num);

  } else {

    const int THREADS = 32;

    PerGaussianRenderPreCUDA<NUM_CHAFFELS>
        <<<((B * 32) + THREADS - 1) / THREADS, THREADS, 0, stream>>>(
            ranges, point_list, W, H, B, per_bucket_tile_offset, bucket_to_tile,
            sampled_T, sampled_ar, bg_color, means2D, conic_opacity, colors,
            final_Ts, n_contrib, max_contrib, pixel_colors, dL_dpixels,
